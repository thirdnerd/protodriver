import type {
  TransferDigestAlgorithm,
  TransferDigestProvider,
...
class Sha256Digest implements TransferStreamingDigest {
  ...
  async digestHex(): Promise<string> {
    ...
    for (let index = 0; index < 8; index += 1) this.#block[63 - index] = Number((bitLength >> BigInt(index * 8)) & 0xffn);
    this.#compress(this.#block);
...
/** Portable streaming digest provider used by both manifest host stacks. */
export const SHA256_TRANSFER_DIGESTS: TransferDigestProvider = Object.freeze({
  create(algorithm: TransferDigestAlgorithm): TransferStreamingDigest {
    if (algorithm !== "sha256") throw new Error(`unsupported transfer digest algorithm ${JSON.stringify(algorithm)}`);

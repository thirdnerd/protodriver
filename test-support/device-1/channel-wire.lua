...
local function receive(io,timer,count)
  local result=io.request({kind="wait-fill",count=count,timers=a({timer.id})})
  if result:sub(1,8)~="receive:" then timer.live=false;fail("device-1.response-timeout") end
  return result:sub(9)
end
function M.delay(io,ms)
  local timer=io.request({kind="timer-arm",milliseconds=ms})
  io.request({kind="wait-any",maximum=0,timers=a({timer})})
end
...
function M.exchange(io,request,shape,ordinary)
  ...
  if deadline then io.request({kind="deadline-disarm",deadline=deadline}) end
  local ok,reply=pcall(function()
    if type(shape)=="string" then
      local raw=receive(io,timer,#shape)
      if raw=="\0" then fail("device-1.device-rejected") end
      if raw~=shape then fail("device-1.unknown-message") end
      return raw
    end
    if type(shape)=="number" then return receive(io,timer,shape) end
    local prefix=receive(io,timer,1)
    if prefix=="\0" then fail("device-1.device-rejected") end
    if prefix~=shape.prefix then fail("device-1.unknown-message") end
    local header=prefix..receive(io,timer,shape.header-1)
    local length=header:byte(shape.header)
    if length>shape.maximum then fail("device-1.frame-bound") end
    return header..(length>0 and receive(io,timer,length) or "")
  end)
  if timer.live then io.request({kind="timer-cancel",timer=timer.id}) end
  if not ok then fail(reply) end
  return reply
end
...
function M.read(io,address,length)
  ...
  if raw:sub(2,5)~=suffix then fail("device-1.response-correlation") end
...
function M.sector(io,base)
  local chunks={}
  for offset=0,4095,64 do chunks[#chunks+1]=M.read(io,base+offset,64) end
...
function M.selected(io)
  ...
  for index=0,14 do
    ...
    if tag=="\x16" then
      if base then fail("device-1.ambiguous-sector") end
      base=0x1000+index*0x1000
    end
  end
  if not base then fail("device-1.missing-sector") end
